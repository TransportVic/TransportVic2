...
database.connect({
  ...
  const allRoutes = routeData.split('\r\n').slice(1).filter(Boolean).map(e => {
  ...
  allRoutes.forEach(route => {
    if (!mergedRoutes[route.routeName]) { mergedRoutes[route.routeName] = {} }
  ...
  await async.map(Object.keys(mergedRoutes), async routeName => {
    const combinedRouteData = mergedRoutes[routeName]
