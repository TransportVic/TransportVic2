...
async function getStation(stopGTFSID) {
  if (stationLoaders[stopGTFSID]) {
    ...
    let metroStation = station.bays.filter(bay => bay.mode === 'regional coach')[0]
...
database.connect({
  ...
  await async.forEach(trips, async trip => {
    ...
    allTrips[tripID] = {
      mode: "regional coach",
      operator: "V/Line Coach",
      routeName: route.routeName,
      shortRouteName: route.shortRouteName || route.routeName,
      tripID,
      routeGTFSID,
  ...
  console.log('Completed loading in ' + Object.keys(allTrips).length + ' V/Line Coach trips')
  process.exit()
})
