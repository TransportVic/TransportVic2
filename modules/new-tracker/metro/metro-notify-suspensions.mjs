...
export async function fetchNotifySuspensions(db, tripDB, ptvAPI, existingTrips) {
  ...
  for (let stop of stopsToUse) {
    let newTDNs = []
    newTDNs.push(...await fetchStop(stop, db, tripDB, ptvAPI, 5, false, tdnsSeen, updatedTrips, existingTrips))
    newTDNs.push(...await fetchStop(stop, db, tripDB, ptvAPI, 5, true, tdnsSeen, updatedTrips, existingTrips))
    ...
    let missingTDNs = (await getTrips(db, tripDB, ptvAPI, station, tdnsSeen)).map(update => update.runID)
...
if (await fs.realpath(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  await fetchNotifySuspensions(database, tripDatabase, ptvAPI)
