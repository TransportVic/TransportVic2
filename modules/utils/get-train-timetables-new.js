...
async function getDeparture(stopGTFSID, db, mode, possibleLines, departureTime, possibleDestinations, direction, live, viaCityLoop) {
  ...
  for (let i = 0; i <= iterations; i++) {
    ...
    if (mode === 'metro train' && viaCityLoop !== null) {
      if (viaCityLoop) {
        timetables = timetables.filter(t => trimFromFSS(t).find(s => s.stopName === 'Flagstaff Railway Station'))
      } else {
        timetables = timetables.filter(t => !trimFromFSS().find(s => s.stopName === 'Flagstaff Railway Station'))
      }
    }
